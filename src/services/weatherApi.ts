...
async function getSunriseSunsetFromAPI(
      ...
      (1 / 15) *
        Math.acos(
          (-Math.tan(latRad) * Math.tan(declination)) /
            Math.sqrt(1 - Math.pow(Math.tan(latRad) * Math.tan(declination), 2)),
        );
  ...
  try {
    ...
    if (!data || !data.results) {
      throw new Error(`Invalid response format from sunrise-sunset.org API`);
    }
    ...
    if (data.status === "OK") {
      ...
      throw new Error(`API returned status: ${data.status || "Unknown error"}`);
    }
  } catch (error) {
